...
*/
	...
	public function writeTranscript($issuerOfficialPost, $certFurtherInfo, $aryExplTxt, $qualificationSought, $teachingInstitute, $awardingInstitute, $programmeOfStudy, $programmeTitle, $langInstr, $langAssmnt, $programmeLevel, $programmeHours, $awardAchieved, $studentName, $dtAward, $aryGradesTrans, $aryClassifier, $blnLocalised){
		...
		$strStyleHTML='
			<STYLE TYPE="text/css">
				...
				.results tr td.second.localised{
					/*width: 144px;*/
				}
				.results tr td.third{
					width: 39.47%;
				}
				.results tr td.third.localised{
					/*width: 568px;*/
				}
				.results tr td.fourth{
					width: 7.76%;
				}
				.results tr td.fourth.localised{
					/*width: 96px;*/
				}
				.results tr td.fifth{
					width: 8.03%;
				}
				.results tr td.fifth.localised{
					/*width: 141px;*/
				}
				.results tr td.sixth{
					width: 7.89%;
				}
				.results tr td.sixth.localised{
					/*width: 105px;*/
				}
		...
		$transResHdrLocalised = '<tr class="header">
							<td class="first heavy localised" width="70">Year
							</td>
							<td class="second heavy localised" width="65">Semester
							</td>
							<td class="second heavy localised" width="65">Module Code
							</td>
							<td class="third heavy localised" width="290">Module Title
							</td>
							<td class="fourth heavy localised" width="45">Credit Hours
							</td>
							<td class="fifth heavy localised" width="85">Marks Obtained<br />(Out of 100)
							</td>
							<td class="sixth heavy localised" width="60">Status & Attempts
							</td>
						</tr>';
		//echo '<table>' . ($blnLocalised ? $transResHdrLocalised : $transResHdr) . '</table>'; die();
		$intColCount = 8;
		if($blnLocalised){
			$this->genTransRowsLocalised($aryGradesTrans, $aryClassifier, $transRows, $classed);
			$intColCount = 6;
			$strCreditsFooter = '
							<td colspan="3" align="right"><B>Total credits gained / Total ECTS credits gained&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</B>
							</td>
							<td colspan="3">' . $this->ECTSTotal * 2 . '
							</td>';
		}
		else{
		...
		//echo '<table>' . $transRows . '</table>'; die();
			...
			<tr>
				<td style="border-bottom-width:0px;">
					<table class="results">
						...
						' . $transRows . '
						<tr>
							<td colspan="'. $intColCount .'" align="right">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							</td>
						</tr>
						<tr class="footer">' .
							$strCreditsFooter .
	...
	*/
	...
	function genTransRowsLocalised($aryGradesTrans, $aryClassifier, &$gradeRows = '', &$classed = ''){
		$this->ECTSTotal=0;
		//$cumulGrade = 0; $sumCredits = 0;
		$semester = '1'; $oldLvl = null;
		foreach($aryGradesTrans as $gradeSet => $grades){
			$aryHeader = explode('.', $gradeSet);
			$session = $aryHeader[2];
			$lvl = $aryHeader[0];
			//echo 'Level: ' . $lvl . ', old level: ' . $oldLvl; //die();
			//var_dump($grades[0]);
			$lvlModuleCount = count($grades[0]);
			$intModulesNotStudent = 0;
			foreach($grades[0] as $moduleCode => $gradeInfo){
				if(!empty($gradeInfo[2][0])){
					//array_filter($grades[0], function($v) { return $v['type'] == 'folder'; })
					if($oldLvl!=$lvl)
					{
						$yearCell = '<td class="first localised" width="70" rowspan="{modCount}">' . $this->aryYearLookup[$lvl] . ' Year<br />' . $session . '</td>';
					}
					else{
						$yearCell = '';
					}
					$gradeRows = $gradeRows . '
								<tr>
									' . $yearCell .
									'<td class="second localised" width="65">' . $semester . '</td>
									<td class="third localised" width="65">' . $moduleCode . '</td>
									<td class="fourth localised" width="290">' . $gradeInfo[0] . '</td>
									<td class="fifth localised" width="45">' . $gradeInfo[1] . '</td>
									<td class="sixth localised" width="85">' . number_format($gradeInfo[2][0], 2) . '</td>
									<td class="seventh localised" width="60">' . $gradeInfo[2][1] . '</td>
								</tr>';
					...
					$intModulesNotStudent++;
				}
				//print '<br />module code: '.$moduleCode;
				//var_dump($gradeInfo);
			}
			$gradeRows = str_replace('{modCount}', $lvlModuleCount - $intModulesNotStudent, $gradeRows);
		}
