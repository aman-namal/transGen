...
$stage = '';
$route = '';
$pathToData = 'data\\fullresult\\' . $_POST['cboGradYear'] . '\\' . $_POST['cboProg'];
//echo $pathToData; die();
//$strFiles = '';
$strOptions = '';
//to-do: add check to ensure that four files are present in the specified folder
    ...
    while (false !== ($entry = readdir($handle))) {
		if($entry!='.' && $entry!='..'){
			...
			if (substr($entry, strpos($entry, '.')+1, 3)=='xls'){
				...
				$aryStudents = getBradfordGrades("$pathToData\\" . $entry, $rowOffset, $colHdrData, $aryColsStudentName, $colAwardAvg, $rowYear, $keyWordYear, $rowRoute, $keyWordRoute, $rowStage, $keyWordStage, $colIndexModuleInfoNumericGuess, $blnStudentsOnly, $year, $stage, $route);
...
<TR>
	...
	<TD><INPUT TYPE='TEXT' ID='txtAwardDt' Value='12/3/2015' /></TD>
</TR>
<TR>
	<TD><LABEL ID='lblCertOfficial'>Authorised By</label></TD>
	<TD><INPUT TYPE='TEXT' ID='txtCertOfficial' Value='Shirley Congdon' /></TD>
</TR>
<TR>
	<TD><LABEL ID='lblPost'>Official Post</label></TD>
	<TD><INPUT TYPE='TEXT' ID='txtIssuerPost' Value='Pro-Vice-Chancellor (Learning and teaching)' /></TD>
</TR>
<TR>
	<TD><LABEL ID='lblFurtherInfo'>Further Information</label></TD>
	<TD><INPUT TYPE='TEXT' ID='txtFurtherInfo' Value='https://edocs.bradford.ac.uk' /></TD>
</TR>
<TR>
